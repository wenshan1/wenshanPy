class HttpServers():
    ...
    def __init__(self, ip='localhost', port=8000, ssl_port=1443, use_ssl=False, use_cgi=False, pem_cert_file='',
                 pem_key_file='', start_browser=False):
        self.web_address = ip
        self.web_port = port
    ...
    class CGIRequestHandler(CGIHTTPRequestHandler):
        ...
        def do_GET(self):
            if '/shutdown/' in self.path:
                ...
                self.end_headers()
                self.wfile.write(
                    bytes('<html><head><title>Shutdown</title></head><body><h2>Web Server Closed.</h2></body></html>',
                          "utf-8"))
                self.shutdown()
            else:
    ...
    def start(self):
        ...
        if self.use_ssl:
            ...
            httpd.socket = HttpServers.ssl_wrap_socket(httpd.socket, server_side=True, keyfile=self.key_file_path,
